...
const CardHome = ({data, currentCard, setCurrentCard}) => {
    return (
        <div className={`lg:h-60 lg:w-80 flex flex-col justify-between text-[rgba(66, 72, 84, 1)] text-base bg-richblack-800 mx-auto -mb-8 ${currentCard === data?.heading ? "bg-white shadow-[10px_10px_0px_0px_rgba(255,214,10,1)] text-richblack-400": ""}`} onClick={() => setCurrentCard(data?.heading)}>
            <div className='pl-8 pr-12 py-4 flex flex-col gap-3 items-start '>
                <h2 className={`text-xl text-white font-bold capitalize ${currentCard === data?.heading && "text-richblack-800"}`}>{data.heading}</h2>
